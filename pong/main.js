...
const letsMoveIt = (directionX, directionY, directionXd, directionYd) => {
                    if( baballCoor.top <= fieldCoor.top){ // if the ball is going outside of top field
                      directionYd = 1;
                    }
                    if (baballCoor.bottom >= fieldCoor.bottom){ // if the ball is goint outside of bottom field
                      directionYd = 0;
                    } 

                    if(baballCoor.left <= paddleOneCoor.right  // if ball is further left than the paddle  + top & bottom conditions
                      &&
